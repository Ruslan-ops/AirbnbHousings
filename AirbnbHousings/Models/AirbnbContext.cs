...
#warning To protect potentially sensitive information in your connection string, you should move it out of source code. You can avoid scaffolding the connection string by using the Name= syntax to read it from configuration - see https://go.microsoft.com/fwlink/?linkid=2131148. For more guidance on storing connection strings, see http://go.microsoft.com/fwlink/?LinkId=723263.
        => optionsBuilder.UseNpgsql("Host=localhost;Port=5466;Database=airbnb;Username=postgres;Password=postgres");

    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        modelBuilder.Entity<Author>(entity =>
        {
            entity.HasKey(e => e.AId).HasName("authors_pkey");
        ...
        {
            entity.HasKey(e => e.BId).HasName("books_pkey");
            ...
            entity.HasMany(d => d.AIds).WithMany(p => p.BIds)
                .UsingEntity<Dictionary<string, object>>(
                    "M2mBooksAuthor",
                    r => r.HasOne<Author>().WithMany()
                        .HasForeignKey("AId")
                        .HasConstraintName("FK_m2m_books_authors_authors"),
                    l => l.HasOne<Book1>().WithMany()
                        .HasForeignKey("BId")
                        .HasConstraintName("FK_m2m_books_authors_books"),
                    j =>
                    {
                        j.HasKey("BId", "AId");
                        j.ToTable("m2m_books_authors", "library");
                        j.IndexerProperty<int>("BId").HasColumnName("b_id");
                        j.IndexerProperty<int>("AId").HasColumnName("a_id");
                    });

            entity.HasMany(d => d.GIds).WithMany(p => p.BIds)
                .UsingEntity<Dictionary<string, object>>(
                    "M2mBooksGenre",
                    r => r.HasOne<Genre>().WithMany()
                        .HasForeignKey("GId")
                        .OnDelete(DeleteBehavior.ClientSetNull)
                        .HasConstraintName("FK_m2m_books_genres_genres"),
                    l => l.HasOne<Book1>().WithMany()
                        .HasForeignKey("BId")
                        .OnDelete(DeleteBehavior.ClientSetNull)
                        .HasConstraintName("FK_m2m_books_genres_books"),
                    j =>
                    {
                        j.HasKey("BId", "GId");
                        j.ToTable("m2m_books_genres", "library");
                        j.IndexerProperty<int>("BId").HasColumnName("b_id");
        ...
        {
            entity.HasKey(e => e.GId).HasName("genres_pkey");
        ...
        {
            entity.HasKey(e => e.SId).HasName("subscribers_pkey");

            entity.ToTable("subscribers", "library");
        ...
        {
            entity.HasKey(e => e.SbId).HasName("subscriptions_pkey");

            entity.ToTable("subscriptions", "library");

            entity.Property(e => e.SbId).HasColumnName("sb_id");
            entity.Property(e => e.SbBook).HasColumnName("sb_book");
            entity.Property(e => e.SbFinish).HasColumnName("sb_finish");
            entity.Property(e => e.SbIsActive)
                .HasMaxLength(1)
                .HasColumnName("sb_is_active");
            entity.Property(e => e.SbStart).HasColumnName("sb_start");
            entity.Property(e => e.SbSubscriber).HasColumnName("sb_subscriber");

            entity.HasOne(d => d.SbBookNavigation).WithMany(p => p.Subscriptions)
                .HasForeignKey(d => d.SbBook)
                .HasConstraintName("FK_subscriptions_books");

            entity.HasOne(d => d.SbSubscriberNavigation).WithMany(p => p.Subscriptions)
                .HasForeignKey(d => d.SbSubscriber)
                .HasConstraintName("FK_subscriptions_subscribers");
        });
